...
| `ROTATION_GRACE_WINDOW`       | How long a just-rotated refresh token's replacement pair stays retrievable by a legitimate near-simultaneous re-presentation, instead of a `401`. `'0'` disables it. | `"5s"`                                                     |
...
**Rotation grace window:** single-use rotation, by itself, has no tolerance for two legitimate
requests that both present the same, still-valid-at-send-time refresh token within a short window of
each other — a browser prefetching a link on hover and then navigating it (`@zanix/space`'s Orbit,
for example), a double click, or two tabs sharing one session. `refreshSessionTokensBase` handles
this: a presented token that's already blocklisted is checked against a short-lived grace cache
before being rejected, and a hit there — the token was rotated moments earlier by another request,
not reused after the fact — returns that same already-issued pair instead of a `401`. The window's
length is controlled by `ROTATION_GRACE_WINDOW` (a TTL string like `'5s'`/`'10s'`, or a bare number
of seconds; defaults to `5s`); set it to `'0'` to disable the window and restore strict,
zero-tolerance rotation.
